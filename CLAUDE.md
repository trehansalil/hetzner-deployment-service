...
- When adding manifests, also update the corresponding `deploy-<app>` Makefile target **and** the `Apply k8s manifests — <app>` step in `.github/workflows/deploy.yml`. The two diverge intentionally on one file: `secret.yaml` is gitignored, so the workflow does not apply it; the Makefile does (the operator runs `make k8s-secrets-<app>` or relies on `deploy-<app>` to apply a locally-present `secret.yaml`). Keep the **non-secret** manifest list in sync between the two.
